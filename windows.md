...
A development environment refers to the set of processes and tools used to write computer software. This can encompass everything from the physical computer you’re working on to any software you have installed and configured to support development such as your code editor, javascript,node.js etc.
...
When this command completes, run the 

`vagrant ssh`

command to log in to Vagrant.
...
If you notice any express servers or Ruby on Rails servers that are still running in another copy or window of terminal, you should quit those as you’ll get an error about multiple Rails instances or EADDRINUSE, Address already in use.

Running the `killall ruby` or `killall node` command in your terminal should quit all running servers if you’re unsure.
